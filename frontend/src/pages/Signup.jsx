...
export default function Signup() {
  ...
  const handleGoogleCredential = async (credential) => {
    setLoading(true);
    try {
      const googlePlan = isStudentFlow ? "student" : isCompanyFlow ? orgTier : plan;
      const res = await googleLogin(credential, googlePlan, isStudentFlow);
      store.setUser(res.user);
      store.setToken(res.token);
      localStorage.setItem("dp_token", res.token);
      localStorage.setItem("dp_user",  JSON.stringify(res.user));
      ...
      if (isCompanyFlow) {
        try {
          const orgRes = await createOrganization(orgName.trim() || `${res.user.name}'s Team`, orgTier, seats);
          toast.success(orgRes.message || "Organization created!");
        } catch (orgErr) {
          toast.error("Signed in, but organization setup failed: " + (orgErr.response?.data?.detail || orgErr.message));
        }
      } else {
