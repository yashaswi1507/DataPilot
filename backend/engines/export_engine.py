...
def export_excel(report_name, rpt_kpis, rpt_charts, insights, raw_data=None):
    """
    Builds a formatted .xlsx report with a Summary sheet (KPIs +
    insights) and, if raw_data is provided (columns + rows), a Data
    sheet with the actual dataset so the user gets a working spreadsheet
    rather than just a static summary. No formulas are generated here —
    this is a point-in-time export, not a live model, so values are
    written directly (matches what the PDF/PPT exports already do).
    """
    ...
    title_font   = Font(name="Calibri", size=18, bold=True, color=PURPLE)
    header_font  = Font(name="Calibri", size=11, bold=True, color="FFFFFF")
    label_font   = Font(name="Calibri", size=10, color=GREY)
    value_font   = Font(name="Calibri", size=14, bold=True, color=DARK)
    body_font    = Font(name="Calibri", size=10, color=DARK)
    header_fill  = PatternFill("solid", start_color=PURPLE)
    kpi_fill     = PatternFill("solid", start_color=LIGHT_BG)
    thin_border  = Border(bottom=Side(style="thin", color="E5E7EB"))

    # Title
    summary["A1"] = report_name
    summary["A1"].font = title_font
    summary.merge_cells("A1:D1")
    summary["A2"] = "Generated by DataPilot"
    summary["A2"].font = label_font
    summary.merge_cells("A2:D2")

    row = 4
    # KPIs as a small grid, 2 per row
    summary.cell(row=row, column=1, value="Key Metrics").font = Font(size=12, bold=True, color=DARK)
    row += 1
    kpi_items = list(rpt_kpis.items())[:8]
    for i in range(0, len(kpi_items), 2):
        pair = kpi_items[i:i+2]
        for j, (k, v) in enumerate(pair):
            col = 1 + j * 2
            label_cell = summary.cell(row=row, column=col, value=k)
            value_cell = summary.cell(row=row+1, column=col, value=v)
            label_cell.font = label_font
            value_cell.font = value_font
            for c in range(col, col+2):
                summary.cell(row=row, column=c).fill = kpi_fill
                summary.cell(row=row+1, column=c).fill = kpi_fill
        row += 3

    # Insights
    if insights:
        row += 1
        summary.cell(row=row, column=1, value="Insights").font = Font(size=12, bold=True, color=DARK)
        row += 1
        for ins in insights[:10]:
            cell = summary.cell(row=row, column=1, value=f"• {ins}")
            cell.font = body_font
    ...
    for col_letter in ["A", "B", "C", "D"]:
        summary.column_dimensions[col_letter].width = 24

    # Data sheet — the actual dataset, if provided
    if raw_data and raw_data.get("columns") and raw_data.get("data"):
        data_sheet = wb.create_sheet("Data")
        columns = raw_data["columns"]
        rows_data = raw_data["data"]

        for col_idx, col_name in enumerate(columns, start=1):
            cell = data_sheet.cell(row=1, column=col_idx, value=str(col_name))
            cell.font = header_font
            cell.fill = header_fill
            cell.alignment = Alignment(horizontal="center")

        # Cap at 50,000 rows to keep file size/generation time reasonable —
        # this is a report export, not a full data warehouse dump.
        for row_idx, row_vals in enumerate(rows_data[:50000], start=2):
            for col_idx, val in enumerate(row_vals, start=1):
                data_sheet.cell(row=row_idx, column=col_idx, value=val)
